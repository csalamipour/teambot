...
class StreamingChannelData:
    ...
    def to_dict(self):
        ...
        result = {"stream_type": self.stream_type}
        ...
        if self.feedback_loop_enabled is not None:
            result["feedback_loop_enabled"] = self.feedback_loop_enabled
            
        if self.feedback_loop_type:
            result["feedback_loop_type"] = self.feedback_loop_type
...
class TeamsStreamingResponse:
    ...
    def queue_next_chunk(self):
        ...
        def _format_next_chunk():
            ...
            self._chunk_queued = False
            if self._ended:
                channel_data = StreamingChannelData(stream_type="final")
                return Activity(
                    type="message",
                    text=self._message,
    ...
    async def send_activity(self, activity):
        ...
        try:
            ...
            channel_data = StreamingChannelData.from_dict(channel_data_dict)

            if self._stream_id:
                channel_data.stream_id = self._stream_id
                activity.channel_data = channel_data.to_dict()
            ...
            logging.error(f"Error sending streaming activity: {e}")
            traceback.print_exc()
            raise
...
async def handle_text_message(turn_context: TurnContext, state):
    ...
    try:
        ...
        with pending_messages_lock:
            if conversation_id in pending_messages and pending_messages[conversation_id]:
                # FIXED: Instead of deep copying the context, create a new Turn Context with necessary info
                ...
                # Process the message directly without copying the context
                # Just pass the original context and update the message text
